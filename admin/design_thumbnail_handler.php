...
// upload ficher //
            ...
            // Check file size
            if ($_FILES["design_thumbnail"]["size"] > 5000000) {
            ...
            // if everything is ok, try to upload file
              ...
              if (move_uploaded_file($_FILES["design_thumbnail"]["tmp_name"], $target_file)) {
                echo "The file ". htmlspecialchars( basename( $_FILES["design_thumbnail"]["name"])). " has been uploaded.";
              } else {
            ...
            $sql ='UPDATE `designs` SET `design_thumbnail`=:design_thumbnail WHERE `iddesign`=:iddesign';


            $query = $db ->prepare($sql);

            $query->bindValue(':iddesign', $id, PDO::PARAM_INT);
            $query->bindValue(':design_thumbnail', $thumb['name'], PDO::PARAM_STR);
            $query->execute();
            echo 'Sucess';
